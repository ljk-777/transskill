...
> 版本: v0.2 | 最后更新: 2026-05-23 | 基于: requirements.md v0.2 + design.md v0.2
...
|:---|:----|:---------|:----|
| **Phase 0** | 项目脚手架 | 0.5 天 | ⬜ |
| **Phase 1** | 核心数据模型 + Resolver | 1.5 天 | ⬜ |
| **Phase 2** | Parser（含目录扫描） | 2 天 | ⬜ |
| **Phase 3** | Mapper + Renderer（含目录渲染） | 2 天 | ⬜ |
| **Phase 4** | CLI 接口 | 1.5 天 | ⬜ |
| **Phase 5** | 测试覆盖 | 1.5 天 | ⬜ |
| **Phase 6** | 工具链完善（CI/发布） | 0.5 天 | ⬜ |
...
| T-111 | 实现解析工具函数 | URL 正则解析（gh: 格式、github.com URL）、临时目录管理 | `src/resolver/utils.ts` | T-110 |
...
- ✅ `GitHubResolver.supports('随便一句')` → false（不匹配时优雅降级）
- ✅ `resolveInput('gh:user/repo')` 返回含 cleanup 的 ResolvedInput
- ✅ 测试中使用 mock 避免真实 git clone
...
| T-203 | 实现 `SkillDirectoryScanner` | 扫描目录，识别 SKILL.md + 附属目录结构 | `src/parser/skill-directory.ts` | T-103 |
...
| ID | 任务 | 实现类 | 核心逻辑 | 测试用例 | 依赖 |
|:--|:-----|:------|:--------|:--------|:----|
| T-204 | SKILL.md Parser | `SKILLMdParser` | gray-matter 解析 → name/description/正文 | 1. 标准 frontmatter<br>2. 无 frontmatter → 报错<br>3. 空 description → fallback | T-201 |
| T-205 | .cursorrules Parser | `CursorRulesParser` | 全文为 instructions，name=文件名 | 1. 标准文件<br>2. 空文件 | T-201 |
| T-206 | .mdc Parser | `MdcParser` | gray-matter → globs/alwaysApply/desc | 1. 标准 .mdc<br>2. 无 frontmatter → 降级 | T-201 |
| T-207 | MCP JSON Parser | `MCPJsonParser` | JSON Schema → tools[] + description | 1. 标准 MCP JSON<br>2. 空 tools → 警告 | T-201 |
| T-208 | SOUL.md Parser | `SOULMdParser` | 解析身份定义 | 1. 标准 SOUL.md | T-201 |

**验收标准：**
- ✅ 每个 Parser 有 ≥1 个 fixture 文件和测试用例
- ✅ `detect()` 正确识别对应格式
- ✅ `parseDirectory()` 正确扫描 skill 目录
- ✅ 非法输入抛出 `SkillBridgeError`
...
**验收标准：**
- ✅ `skillbridge convert sample.cursorrules --to skill.md` → 合法 SKILL.md
- ✅ `skillbridge convert sample-skill/ --to .cursorrules` → 完整目录输出
- ✅ `skillbridge convert gh:user/test-skill --to .cursorrules` → GitHub 模式成功
- ✅ `skillbridge convert gh:user/test-skill --to skill.md --install-to ~/.claude/skills/` → 直接安装
- ✅ `skillbridge list-formats` → 列出所有格式
- ✅ `skillbridge validate sample.skill.md` → 验证通过/报错
- ✅ `skillbridge convert xxx --dry-run` → 不写入任何文件
- ✅ 错误时红色输出，退出码非 0
