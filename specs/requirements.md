...
### 1.1 一句话描述
SkillBridge 是一个跨 AI Agent 平台的技能格式转换与分发工具——技能写一次，在任何 Agent 上都能跑。
...
| US-004 | 开发者 | 我希望通过命令行 `skillbridge convert --help` 查看所有转换选项 | 1. 显示所有支持的输入/输出格式<br>2. 显示示例用法<br>3. 显示可选参数说明 |
...
| US-006 | 开发者 | 我希望直接从 GitHub 仓库转换 skill，而不需要先手动 clone | 1. `skillbridge convert gh:user/repo --to .cursorrules`<br>2. 工具自动 clone 到临时目录<br>3. 自动检测仓库中的技能文件<br>4. 转换完毕后清理临时文件<br>5. 对公共仓库零配置 |
...
| US-010 | 开发者 | 我希望批量转换一个目录下的多个技能 | 1. `skillbridge convert ./skills/ --to .cursorrules`<br>2. 自动递归处理所有技能子目录<br>3. 输出到指定目录，保持原文件名/目录名 |
...
```bash
# 一条命令搞定
$ skillbridge convert gh:anthropics/skills/weather --to skill.md \
...
```bash
$ skillbridge convert gh:user/ts-rules --to .mdc \
  --glob "**/*.ts" \
...
```bash
$ skillbridge convert ./mcp-server.json --to skill.md -o ./skills/
