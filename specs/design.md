...
│  skillbridge convert <input> [options]           │
│  skillbridge --version / --help                  │
└──────────────────────┬──────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────┐
│             InputResolver 层                      │  ← 新增
...
InputResolver: GitHubResolver
  ├── git clone --depth 1 到 /tmp/skillbridge-xxx/
  └── 返回本地路径 /tmp/skillbridge-xxx/weather-skill/
      │
      ▼
...
class GitHubResolver implements InputResolver {
  ...
  async resolve(input: string): Promise<ResolvedInput> {
    ...
    // 2. 在临时目录 clone
    const cloneDir = await mkdtemp(path.join(os.tmpdir(), 'skillbridge-'));
...
class CursorRulesRenderer implements Renderer {
  ...
  render(skill: IntermediateSkill): string {
    const header = [
      ...
      `# ${skill.description}`,
      '#',
      `# Generated by SkillBridge from ${skill.metadata.sourceFormat}`,
      '',
    ].join('\n');
...
$ skillbridge convert .cursorrules --to skill.md -o my-skill.md

# 单文件：SKILL.md → .cursorrules
$ skillbridge convert my-skill.skill.md --to .cursorrules

# === 完整 skill 目录 ===

# 完整 skill 目录 → Cursor 规则
$ skillbridge convert ./weather-skill/ --to .cursorrules

# SKILL.md → .mdc（带文件范围限定）
$ skillbridge convert my-skill.skill.md --to .mdc \
  --glob "**/*.ts" \
  --always-apply \
  -o .cursor/rules/
...
# GitHub 仓库 → 当前目录
$ skillbridge convert gh:user/weather-skill --to .cursorrules

# GitHub 子路径 → 直接安装到 Claude Code
$ skillbridge convert gh:user/skills-repo/weather --to skill.md \
  --install-to ~/.claude/skills/

# GitHub 完整 URL
$ skillbridge convert https://github.com/user/weather-skill --to .mdc

# === 其他 ===

# 查看所有支持的格式
$ skillbridge list-formats

# dry-run：看看会做什么，不实际执行
$ skillbridge convert gh:user/weather-skill --to .cursorrules --dry-run

# 验证文件格式
$ skillbridge validate my-skill.skill.md
```
...
$ skillbridge convert gh:user/weather-skill --to skill.md \
  --install-to ~/.claude/skills/
...
$ skillbridge convert gh:user/weather-skill --to .mdc \
...
```typescript
class SkillBridgeError extends Error {
  constructor(
    message: string,
